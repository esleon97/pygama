...
The pygama processors use the
[ufunc](https://docs.scipy.org/doc/numpy/reference/ufuncs.html) framework,
which is designed to encourage highly performant python practices. Ufuncs have
several advantages:

1. Ufuncs work with [numpy
arrays](https://docs.scipy.org/doc/numpy/reference/arrays.html). Numpy arrays
...
for representing waveforms and transformed waveforms.

2. Ufuncs perform vectorized operations. Vectorization causes commands to
perform the same operation on all components of an array. For example, the
ufunc `np.add(a, b, out=c)` (equivalently `c=a+b`) is equivalent to:
```
...
Furthermore, ufuncs are capable of
[broadcasting](https://docs.scipy.org/doc/numpy/reference/ufuncs.html#broadcasting)
their dimensions. This involves a safety check to ensure the dimensions of `a`
and `b` are compatible sizes. It also will automatically replicate a size-1
...
One of the biggest advantages of vectorized ufuncs is that many of them will
...
3. Ufuncs are capable of calculating their output in place, meaning they can
...
this can be done using the out keyword in arguments (ex `np.add(a, b, out=c)`,
or more succinctly, `np.add(a, b, c)`). While this may seem counterintuitive at
first, the alternative (`c = np.add(a,b)` or `c = a+b`) causes an entirely new
array to be allocated, with c pointing at that. These array allocations can add
up very quickly: `e = a*b + c*d`, for example, would allocate 3 different
arrays: one for `a*b`, one for `c*d`, and one for the sum of those two. As we
write ufuncs to use for LEGEND, it is important that we try to use functions
that operate in place as much as possible!
"""
